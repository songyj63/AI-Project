...
# load data
X_training = np.load('Data/X_training_Normalised_balanced_shuffled.npy')
X_testing = np.load('Data/X_testing_Normalised.npy')
labelSet_training = np.load('Data/labelSet_training_balanced_shuffled.npy')
...
# L1 = tf.layers.conv2d(X, 4, [3, 3], padding='same', activation=tf.nn.selu, kernel_initializer=tf.random_normal_initializer, bias_initializer=tf.random_normal_initializer)
...
# #    Pool     ->(?, 75, 19, 64)
# L2 = tf.layers.conv2d(L1, 8, [2, 2], padding='same', kernel_initializer=tf.random_normal_initializer, bias_initializer=tf.random_normal_initializer)
...
# #    Pool     ->(?, 25, 6, 32)
# L3 = tf.layers.conv2d(L2, 16, [3, 2], padding='same', activation=tf.nn.leaky_relu, kernel_initializer=tf.random_normal_initializer, bias_initializer=tf.random_normal_initializer)
...
# # L3 Conv shape=(?, 75, 19, 16)
# #    Pool     ->(?, 25, 6, 32)
# L4 = tf.layers.conv2d(L3, 8, [2, 3], padding='same', kernel_initializer=tf.random_normal_initializer, bias_initializer=tf.random_normal_initializer)
...
# L5 = tf.layers.dense(L5, 10, activation=tf.nn.leaky_relu, kernel_initializer=tf.random_normal_initializer, bias_initializer=tf.random_normal_initializer)
...
# optimizer = tf.train.AdamOptimizer(0.0001).minimize(cost, global_step=global_step)
#
#
# sess = tf.Session()
...
# print('done')
# print('정확도 1: %.2f' % sess.run(accuracy * 100, feed_dict={X: X_testing[0:3500, :, :, :], Y: label_testing[0:3500, :], is_training: False}))
# print('정확도 2: %.2f' % sess.run(accuracy * 100, feed_dict={X: X_testing[3500:7000, :, :, :], Y: label_testing[3500:7000, :], is_training: False}))
#
#
#
